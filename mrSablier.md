...
# Latest production code review 


```
mrSablier Re-Review — commit 38a4d0b "Suggested changes"
  ...
  │  #  │              Issue               │                                                      Status                                                      │
  ├─────┼──────────────────────────────────┼──────────────────────────────────────────────────────────────────────────────────────────────────────────────────┤
  │ 3   │ Stale blockhash reused across    │ FIXED — blockhash now fetched per endpoint inside the loop                                                       │
  │     │ RPC fallback retries             │                                                                                                                  │
  ├─────┼──────────────────────────────────┼──────────────────────────────────────────────────────────────────────────────────────────────────────────────────┤
  │ 4   │ No owner validation on fetched   │ FIXED — rpc_fallback.get_account() now validates account.owner == adrena_abi::ID before deserialization; all     │
  │     │ custody accounts                 │ call sites including process_stream_message.rs:79 get this automatically                                         │
  ├─────┼──────────────────────────────────┼──────────────────────────────────────────────────────────────────────────────────────────────────────────────────┤
  │ 7   │ Silent timestamp parse failure   │ FIXED — now logs full context (provider, feed_id, malformed value, error) before returning 0                     │
  │     │ returns 0                        │                                                                                                                  │
  └─────┴──────────────────────────────────┴──────────────────────────────────────────────────────────────────────────────────────────────────────────────────┘
  ...
  │       #        │                 Issue                 │                                             Status                                              │
  ├────────────────┼───────────────────────────────────────┼─────────────────────────────────────────────────────────────────────────────────────────────────┤
  │ 4 (confirm     │ 1.5s confirmation window misses valid │ PARTIAL — bumped from CONFIRM_POLLS=3 to 4 (1.5s → 2.0s). Still well below the 5–10s needed     │
  │ poll)          │  txs                                  │ under congestion                                                                                │
  └────────────────┴───────────────────────────────────────┴─────────────────────────────────────────────────────────────────────────────────────────────────┘

  ---
  Still Present ❌

  1. oracle_onchain_fallback.rs:54–67 — Bounds check after slice access, unwrap() panics ❌ CRITICAL

  Unchanged. The break guard fires for future iterations but the .try_into().unwrap() calls at lines 60, 65, 66, 67 are still before any safety net on corrupted
  data.

  2. process_stream_message.rs:115–120 — Explicit panic! on stream filter states ❌ CRITICAL

  PositionUpdate::Created(_) => {
      panic!("New position created in positions_close filter");
  }
  PositionUpdate::Modified(_) => {
      panic!("New position created in positions_close filter"); // wrong msg too
  }
  Both panics and the duplicated wrong error message unchanged.

  3. handlers/liquidate_short.rs:63 — Hardcoded mock USDC collateral price ❌ CRITICAL

  let mock_collateral_token_price = OraclePrice::new(1_000_000, -6, 0, current_time, ...);
  Still hardcoded. Wrong liquidation decisions if collateral ≠ USDC or USDC depegs.

  4. transaction_wrapper.rs:62–67 — Limit order zeroed not removed ❌ HIGH

  *order = adrena_abi::types::LimitOrder::default(); // zeroed, stays in vec
  Zero-valued entries accumulate in the limit_orders Vec indefinitely.

  5. transaction_wrapper.rs:55–59 — Race condition in position cleanup ❌ HIGH

  Unchanged. Stream update arriving between tx success and remove() can delete a freshly re-created position entry.

  6. handlers/liquidate_long.rs:338–349 — Priority fee tier threshold scale mismatch ❌ HIGH

  MAX_FEE_CALC_SIZE = 2_500_000_000000 ($2.5M) but ULTRA tier fires at loss > 2_500_000000 ($2,500). The tiers appear to be checking at the wrong decimal scale.

  7. jito_bundle.rs:93, 181 — .unwrap() on system time and tx serialization ❌ MEDIUM

  Both unchanged.

  8. handlers/common.rs:379–396 — No backoff on oracle retry signal ❌ MEDIUM

  Returns backoff::Error::transient immediately with no delay — stampedes datapi on sustained oracle staleness.

  ---
  Summary

  ┌───────┬─────────┬───────────────┐
  │ Fixed │ Partial │ Still Present │
  ├───────┼─────────┼───────────────┤
  │ 3     │ 1       │ 8             │
  └───────┴─────────┴───────────────┘

  3 of the 4 critical bugs from the original review are still open (oracle_onchain_fallback unwraps, stream panic!, hardcoded short collateral price). The commit
  correctly addressed the blockhash staleness, owner validation, and timestamp logging — solid improvements — but the service-crash vectors in
  oracle_onchain_fallback.rs and process_stream_message.rs remain blockers before release.
```
